...
- **`.claude/skills/`** — the skills, and the working **source of truth**. Each is a self-contained Agent Skill: a `SKILL.md` spine plus optional `references/`, `templates/`, `scripts/`. Manual experiments stay top-level so explicit invocation remains discoverable; register them in `IN_PROCESS_SKILLS` in `sync_plugin.py` to exclude them from every distribution target.
